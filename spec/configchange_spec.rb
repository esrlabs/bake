...
describe "Config Change" do
  ...
  it 'Change Compiler Flags GCC_ENV' do
    ENV["CXXFLAGS"] = "-DGAGA"
    Bake.startBake("configchanged/main", ["Debug"])
    expect($mystring.include?("Compiling lib (lib): src/x.cpp")).to be == true
    expect($mystring.include?("Creating  lib (lib): build/lib_main_Debug/liblib.a")).to be == true
    expect($mystring.include?("Linking   main (Debug): build/Debug/main"+Bake::Toolchain.outputEnding)).to be == true
    expect(ExitHelper.exit_code).to be == 0
  end
  ...
  it 'Change Archiver Flags GCC_ENV' do
    ENV["ARFLAGS"] = "-s"
    Bake.startBake("configchanged/main", ["Debug"])
    expect($mystring.include?("Compiling lib (lib): src/x.cpp")).to be == false
    expect($mystring.include?("Creating  lib (lib): build/lib_main_Debug/liblib.a")).to be == true
    expect($mystring.include?("Linking   main (Debug): build/Debug/main"+Bake::Toolchain.outputEnding)).to be == true
    expect(ExitHelper.exit_code).to be == 0
  end
  ...
  it 'Change Linker Flags GCC_ENV' do
    ...
    Bake.startBake("configchanged/main", ["Debug"])
    expect($mystring.include?("Compiling lib (lib): src/x.cpp")).to be == false
    expect($mystring.include?("Creating  lib  (lib): build/lib_main_Debug/liblib.a")).to be == false
    expect($mystring.include?("Linking   main (Debug): build/Debug/main"+Bake::Toolchain.outputEnding)).to be == true
    expect(ExitHelper.exit_code).to be == 0
  end
  ...
  it 'Change main config file GCC' do
    ...
    Bake.startBake("configchanged/main", ["Test"])
    expect($mystring.include?("Compiling lib (lib): src/x.cpp")).to be == true
    expect($mystring.include?("Creating  lib (lib): build/lib_main_Test/liblib.a")).to be == true
    expect($mystring.include?("Linking   main (Test): build/Test/main"+Bake::Toolchain.outputEnding)).to be == true
    expect(ExitHelper.exit_code).to be == 0
  end
