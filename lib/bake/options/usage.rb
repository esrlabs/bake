...
module Bake
  ...
  class Usage
    ...
    def self.show
      ...
      puts " --file-list=json         Writes all sources and headers used for a SINGLE config into '<config output folder>/file-list.json'."
      puts "                          Writes all sources and headers used for ALL configs into '<main config output folder/global-file-list.json'."
      puts " --file-list=txt          Plain text files instead of json files."
